# -*- coding: utf-8 -*-
"""Consultation_NLP_interest.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1zTMY2uNM9pnHwPWZmfBpmZgtcVJ35Jti
"""
...
in_path  = '/content/drive/MyDrive/consu_interest/fedlex-data-admin-ch-eli-dl-proj-2022-44-cons_1-doc_6-fr-pdf-a.pdf'
tmp_path = '/content/drive/MyDrive/consu_interest/unsigned.pdf'
...
input_pdf  = '/content/drive/MyDrive/consu_interest/unsigned.pdf'
output_pdf = '/content/drive/MyDrive/consu_interest/ocr_output.pdf'
...
reader = PdfReader('/content/drive/MyDrive/consu_interest/ocr_output.pdf')
...
# 1) Base de sortie
base_out = os.path.expanduser('/content/drive/MyDrive/consu_interest/Extracted_Chapters')
...
src_root = Path('/content/drive/MyDrive/consu_interest/Extracted_Chapters').expanduser()
fr_root  = Path('/content/drive/MyDrive/consu_interest/Extracted_French').expanduser()
de_root  = Path('/content/drive/MyDrive/consu_interest/Extracted_German').expanduser()
it_root  = Path('/content/drive/MyDrive/consu_interest/Extracted_Italian').expanduser()
...
# --- 4) Boucle principale de traitement (sans validation interne) ---
for idx, row in df.iterrows():
    ...
    logger.info(f"Traitement {lang}/{fname} (ligne {idx})")
    ...
    if idx % SAVE_INTERVAL == 0 and idx > 0:
        ...
        logger.info(f"Autosave au idx={idx}")
...
df[["volet1", "volet2"]] = df["Avis"] \
    .str.extract(pattern, flags=re.IGNORECASE | re.DOTALL)

# --- 6) Sauvegarde finale ---
try:
    df.to_excel(output_excel, index=False, engine="openpyxl")
    logger.info(f"Fichier final sauvegardé : {output_excel}")
...
for volet in ['volet1_fr', 'volet2_fr']:
    ...
    # 5) Tracé du camembert (UNE seule appel à pie)
    wedges, texts, autotexts = ax.pie(
        counts,
        labels=None,
        autopct='%1.1f%%',
